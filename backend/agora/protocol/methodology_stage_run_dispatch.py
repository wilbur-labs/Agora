"""One-shot process dispatch for an already claimed later methodology Run."""
...
class MethodologyStageRunDispatchPolicyCheck(ProtocolModel):
    check: Literal[
        "claimed_formal_run",
        "context_binding",
        "current_route",
...
class MethodologyStageRunDispatchPolicyDecision(HashSealedModel):
    """Per-Run allow/block policy for one already claimed later Stage."""
    ...
    stage_run_claim_receipt_id: StableId
    stage_run_claim_receipt_sha256: Sha256Hex
    ...
    checks: list[MethodologyStageRunDispatchPolicyCheck] = Field(
        min_length=6,
        max_length=6,
    )
    dispatchable: bool
    ...
    def validate_policy_decision(self):
        ...
        if self.decision_id != expected_id:
            raise ValueError(
                "methodology Stage dispatch policy identity must match its Run"
            )
        expected = {
            "claimed_formal_run",
            "context_binding",
        ...
        if (
            ...
            raise ValueError(
                "methodology Stage dispatch policy must contain every canonical check"
            )
        if self.dispatchable != all(item.satisfied for item in self.checks):
            raise ValueError(
                "methodology Stage dispatch policy result must match all checks"
            )
        usage_check = next(
        ...
        if usage_check.satisfied != (self.token_reservation > 0):
            raise ValueError(
                "methodology Stage dispatch usage check differs from its reservation"
            )
        if self.blockers != [
            item.detail for item in self.checks if not item.satisfied
        ]:
            raise ValueError(
                "methodology Stage dispatch blockers differ from its result"
            )
        if self.runtime_capabilities != sorted(set(self.runtime_capabilities)):
            raise ValueError(
                "methodology Stage dispatch capabilities must be canonical"
            )
        if self.context_pack_id != f"context:{self.run_id}":
            raise ValueError(
                "methodology Stage dispatch Context identity must match its Run"
            )
        return self


class MethodologyStageRunDispatchClaim(HashSealedModel):
    """Durable single-use spawn attachment for one later formal Run."""
    ...
    def validate_dispatch_binding(self):
        ...
        if self.dispatch_id != expected_id:
            raise ValueError(
                "methodology Stage dispatch identity must match its Run"
            )
        if self.context_pack_id != f"context:{self.run_id}":
            raise ValueError(
                "methodology Stage dispatch Context identity must match its Run"
        ...
        if (
            ...
            or policy.runtime_registry_sha256
            != self.runtime_registry_sha256
            or policy.stage_gate_receipt_id != self.stage_gate_receipt_id
            or policy.stage_gate_receipt_sha256
            != self.stage_gate_receipt_sha256
            or policy.stage_run_claim_receipt_id
            != self.stage_run_claim_receipt_id
            or policy.stage_run_claim_receipt_sha256
            != self.stage_run_claim_receipt_sha256
            or policy.predecessor_dispatch_receipt_id
            != self.predecessor_dispatch_receipt_id
            or policy.predecessor_dispatch_receipt_sha256
            != self.predecessor_dispatch_receipt_sha256
            ...
            or preflight.runtime_registry_sha256
            != self.runtime_registry_sha256
            ...
            raise ValueError(
                "methodology Stage dispatch preflight differs from its claim"
            )
        if self.recovery_not_before <= self.claimed_at:
            raise ValueError(
                "methodology Stage dispatch recovery lease must follow its claim"
            )
        return self


class MethodologyStageRunDispatchReceipt(HashSealedModel):
    """Terminal receipt for one later-Stage process and protocol settlement."""

    schema_version: Literal["1.0"] = "1.0"
    receipt_id: StableId
    settled_at: AwareDatetime
    dispatch_claim: MethodologyStageRunDispatchClaim
    ...
    def validate_terminal_binding(self):
        ...
        if self.receipt_id != (
            f"methodology-stage-dispatch-receipt:{claim.dispatch_id}"
        ):
            raise ValueError(
                "methodology Stage dispatch receipt identity differs"
            )
        if self.settled_at < claim.claimed_at:
            raise ValueError(
                "methodology Stage dispatch receipt predates its claim"
            )
        if (
            ...
            raise ValueError(
                "methodology Stage terminal facts cross their Run binding"
            )
        launch_failed = state.process_status == ProcessStatus.LAUNCH_FAILED
        if launch_failed != (not self.process_started):
            raise ValueError(
                "methodology Stage process-start fact differs from protocol state"
            )
        if self.process_started != (self.pid is not None):
            raise ValueError(
                "methodology Stage dispatch PID differs from process start"
            )
        if not self.process_started and (
            ...
            raise ValueError(
                "an unstarted methodology Stage process requires zero usage"
            )
        if (self.handoff_pack_id is None) != (self.handoff_pack_sha256 is None):
            raise ValueError(
                "methodology Stage Handoff identity and hash must appear together"
            )
        if (
            ...
            raise ValueError(
                "methodology Stage Artifact and Evidence ids must be canonical"
            )
        return self
