...
- 🏛 **チャットボットではなくカウンシル** — 複数のAIエージェントが異なる角度（リサーチ、アーキテクチャ、批評、セキュリティ）から問題を議論し、統一された結論を導きます。
- 🔧 **議論 → 実行** — エージェントは助言するだけではありません。議論後、エクゼキューターが実際のツール（ファイルI/O、シェルコマンド）を呼び出して計画を実行します。
- 🧠 **自己進化** — すべての議論と実行が再利用可能なスキルに変換されます。エージェントはあなたのワークフローに合わせて進化します。
- ⚙️ **完全カスタマイズ可能** — カスタムプロンプトで独自のエージェントを作成。エージェントごとに異なるLLMを指定可能。YAMLで設定、コード変更不要。
- 🔌 **モデル非依存** — Azure OpenAI、OpenAI、Kiro CLI、Claude CLI、Gemini CLI、または任意のOpenAI互換API。エージェントごとに混在可能。
- 🐳 **セルフホスト** — `docker compose up` 一つでスタック全体を所有。ベンダーロックインなし、データはネットワーク外に出ません。
